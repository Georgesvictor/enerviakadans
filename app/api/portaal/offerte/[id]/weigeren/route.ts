...
export async function POST(
  ...
  const { data: o } = await supabase
    .from("quotations")
    .select("id, contact_id, company_id, tenant_id")
    .eq("id", id)
  ...
  await supabase
    ...
    .update({
      status: "geweigerd",
      klant_geweigerd_op: new Date().toISOString(),
    })
    .eq("id", id);

  await supabase.from("audit_log").insert({
    tenant_id: session.tenant_id,
    actie: "klantportaal_offerte_geweigerd",
    metadata: { quotation_id: id, email: session.email },
  });

  return NextResponse.redirect(`${req.nextUrl.origin}/portaal/dashboard`, {
    status: 303,
  });
}
