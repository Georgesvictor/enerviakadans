...
export async function POST(
  ...
  try {
    ...
    const [quoteRes, tenant] = await Promise.all([
      supabase
        .from("quotations")
        .select(
          "*, contacts(voornaam,achternaam,adres_straat,adres_nummer,postcode,gemeente), companies(naam,btw_nummer,adres_straat,adres_nummer,postcode,gemeente), quotation_lines(*)",
        )
        .eq("id", params.id)
    ...
    const o = quoteRes.data;
    if (!o || !tenant)
      return NextResponse.json({ error: "niet gevonden" }, { status: 404 });

    // Haal business_entity op (uit offerte óf default)
    let entity: any = null;
    if (o.business_entity_id) {
      const r = await supabase
        .from("business_entities")
        .select("*")
        .eq("id", o.business_entity_id)
        .maybeSingle();
      entity = r.data;
    }
    if (!entity) {
    ...
    const regels = ((o.quotation_lines as any[]) ?? []).sort(
    ...
    const klantNaam =
      ...
      (o.contacts
        ...
        : "Onbekend");
    const klantAdres =
      (o.companies?.adres_straat ?? o.contacts?.adres_straat) &&
      `${o.companies?.adres_straat ?? o.contacts?.adres_straat} ${o.companies?.adres_nummer ?? o.contacts?.adres_nummer ?? ""}`;
    ...
    const element = React.createElement(DocumentPDF, {
      ...
      doc: {
        ...
        voorwaarden: o.voorwaarden ?? entity?.algemene_voorwaarden ?? null,
        begeleidende_tekst_html: o.begeleidende_tekst_html,
        iban: entity?.iban,
        bic: entity?.bic,
      ...
      klant: {
        naam: klantNaam,
        btw_nummer: o.companies?.btw_nummer,
        adres: klantAdres ?? undefined,
        postcode: o.companies?.postcode ?? o.contacts?.postcode ?? undefined,
        gemeente: o.companies?.gemeente ?? o.contacts?.gemeente ?? undefined,
      },
      regels: regels.map((r) => ({
    ...
    const path = `offertes/${ctx.tenantId}/${params.id}.pdf`;
    ...
    return new NextResponse(buffer as any, {
      ...
      headers: {
        ...
        "content-disposition": `inline; filename="offerte-${o.nummer ?? params.id}.pdf"`,
        "x-signed-url": signed?.signedUrl ?? "",
      },
    });
  } catch (err) {
